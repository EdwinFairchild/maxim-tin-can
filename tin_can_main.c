...
static void datsCccCback(attsCccEvt_t *pEvt)
{
    ...
    // TODO : check if this block of code is needed or not
...
static void tincanProcCccState(tinCantMsg_t *pMsg)
{
    // TODO : check if this block is needed or not 
 ...
 */
/*************************************************************************************************/
static void tincanAppClose(dmEvt_t *pMsg)
{
    /* stop mcs measurement */
    ButtonCheckStop((dmConnId_t) pMsg->hdr.param);
}
...
static void datsProcMsg(dmEvt_t *pMsg)
{
    ...
    switch(pMsg->hdr.event) {
        case ATTS_CCC_STATE_IND:
            tincanProcCccState((tinCantMsg_t*)pMsg);
            break;
...
static const tincanCfg_t mcsMcsCfg = {
...
void DatsHandlerInit(wsfHandlerId_t handlerId)
{
    ...
    tincanInit(handlerId, (tincanCfg_t *) &mcsMcsCfg);
}
    ...
    if ((connId = AppConnIsOpen()) != DM_CONN_ID_NONE)
    {
        switch (btn) {
            ...
            case APP_UI_BTN_2_SHORT: 
                setAndNotify(btn);
                xTaskNotify( button_actions_hdl, (1<<2), eSetBits  );
      ...
      /* Not connected state */  
        ...
        switch (btn) {
            case APP_UI_BTN_1_SHORT:
                /* start advertising */
                //AppAdvStart(APP_MODE_AUTO_INIT);
                APP_TRACE_INFO0("Btn 1 s\r\n");
                break;

            case APP_UI_BTN_1_MED:
                APP_TRACE_INFO0("Btn 1 m\r\n");
                /* Enter bondable mode */
                //AppSetBondable(TRUE);
                break;
            ...
            case APP_UI_BTN_2_EX_LONG:
                ...
                xTaskNotify( button_actions_hdl, (1<<2), eSetBits  );
                setAndNotify(btn);
                //xTaskNotifyGive(button_2_handle);
                break;
